  ...
  return (
    <div className="auth-card w-full max-w-sm rounded-2xl p-8 shadow-[0_20px_60px_rgba(0,0,0,0.35),0_8px_24px_rgba(0,0,0,0.2)]">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <span className="auth-text-subtle text-xs">or sign up with email</span>
        <div className="auth-divider flex-1 h-px" />
      </div>
      ...
      <form action={action} className="space-y-4">
        <div>
          <label htmlFor="displayName" className="auth-text-muted block text-xs font-medium mb-1.5">
            Full Name
          </label>
          <input
            id="displayName"
            ...
            autoComplete="name"
            required
            className={`auth-input auth-text w-full rounded-xl px-4 py-2.5 text-sm outline-none transition-all duration-200 ${state?.errors?.displayName ? 'auth-surface-error' : 'auth-surface'}`}
          />
          {state?.errors?.displayName && (
            <p className="auth-text-error mt-1.5 text-xs">{state.errors.displayName[0]}</p>
          )}
        </div>
        ...
        <div>
          <label htmlFor="password" className="auth-text-muted block text-xs font-medium mb-1.5">
            Password
          </label>
          <input
            id="password"
          ...
          {state?.errors?.password && (
            <ul className="mt-2 space-y-1">
              {state.errors.password.map((err) => (
                <li key={err} className="auth-text-error text-xs">• {err}</li>
              ))}
            </ul>
          )}
        </div>

        <p className="auth-text-subtle text-xs leading-relaxed">
          By signing up, you agree to our{' '}
          <Link href="/terms" className="underline hover:auth-text-crimson transition-colors">Terms</Link>
          {' '}and{' '}
          <Link href="/privacy" className="underline hover:auth-text-crimson transition-colors">Privacy Policy</Link>.
        </p>
      ...
      <p className="auth-text-subtle text-center text-xs mt-6">
        ...
        <Link href="/login" className="auth-text-crimson font-medium hover:underline transition-colors">
          Sign in
        </Link>
      </p>
    </div>
